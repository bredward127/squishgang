...
export function Sidebar({ activeCategory = '' }: { activeCategory?: string }) {
  ...
  const currentSort = searchParams.get('sort') || 'bestsellers';
  ...
  return (
    <aside className="w-full md:w-64 bg-white border-r border-slate-200 p-6 flex flex-col gap-6 h-full min-h-[500px]">
      <div>
        <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3">Sort By</h3>
        <select
          value={currentSort}
          onChange={(e) => handleSortChange(e.target.value)}
          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-pink-300"
        >
